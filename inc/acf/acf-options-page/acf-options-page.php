...
class acf_options_page_plugin
{
	...
	*/
	...
	function __construct()
	{
		// vars
		$this->settings = array(
			'title' 		=> __('EloBoost Options','acf'),
			'menu'			=> __('EloBoost Options','acf'),
			'slug' 			=> 'acf-options',
	...
	*/
	...
	function admin_menu()
	{
		...
		if( $this->settings['show_parent'] )
		{
			...
			$parent_page = add_menu_page( $this->settings['title'], $this->settings['menu'], $this->settings['capability'], $this->settings['slug'], array($this, 'html'), 'dashicons-screenoptions', 50);
	...
	*/
	...
	function admin_head()
	{
		...
		wp_enqueue_style( 'set_admin_css', get_template_directory_uri() . '/framework/admin/acf-options-page/admin.css' );
	...
	*/
	...
	function html()
	{
		...
		<div class="wrap no_move">
			...
			<div class="metabox-holder has-right-sidebar2" id="poststuff">
				...
				<div id="post-body-content">
					...
					<div id="dp-footer">
							...
							<input type="submit" class="acf-button" value="<?php _e("Save Theme Options",'acf'); ?>" />
						</div>
					...
					(function($){
						...
						$('#dp-wrapper h3.hndle span').append('<span class="version">' + "Theme Version <?php echo $my_theme->get( 'Version' ) ?>" + '</span>');
